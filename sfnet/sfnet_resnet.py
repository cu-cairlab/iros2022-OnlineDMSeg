...
class AlignNetResNet(nn.Module):
    def __init__(self, num_classes, trunk='resnet-101', criterion=None, variant='D',
                 skip='m1', skip_num=48, fpn_dsn=False,pretrained=True):
        ...
        if trunk == trunk == 'resnet-50-deep':
            ...
            resnet = Resnet_Deep.resnet18(pretrained=pretrained)
        else:
...
def DeepR18_SF_deeply(num_classes, criterion,pretrained=True):
    """
    ResNet-18 Based Network
    """
    return AlignNetResNet(num_classes, trunk='resnet-18-deep', criterion=criterion, variant='D', skip='m1',pretrained=pretrained)
