...
class AlignNetDFnet(nn.Module):
    ...
    def __init__(self, num_classes, trunk='dfv1', criterion=None, variant='D',
                 skip='m1', skip_num=48, flow_conv_type="conv", fpn_dsn=False,pretrained=True):
        ...
        if trunk == 'dfv1':
            self.backbone = DFNetv1(pretrained=pretrained, norm_layer=Norm2d)
        elif trunk == 'dfv2':
            self.backbone = DFNetv2(norm_layer=Norm2d,pretrained=pretrained)
        else:
...
def AlignedDFnetv2_FPNDSN(num_classes,  criterion,pretrained=True):
    return AlignNetDFnet(num_classes, trunk='dfv2', criterion=criterion, fpn_dsn=True,pretrained=pretrained)
