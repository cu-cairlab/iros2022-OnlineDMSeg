...
class ResNet_Bottleneck_OS16(nn.Module):
    def __init__(self, num_layers,pretrained=False):
        ...
        if num_layers == 50:
            ...
            if pretrained:
                resnet.load_state_dict(torch.load("/media/ignored/resnet50-19c8e357.pth"))
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("/media/ignored/resnet101-5d3b4d8f.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("/media/ignored/resnet152-b121ed2d.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
class ResNet_BasicBlock_OS16(nn.Module):
    def __init__(self, num_layers,pretrained=False):
        ...
        if num_layers == 18:
            ...
            # load pretrained model:
            if pretrained:
                resnet.load_state_dict(torch.load("/media/ignored/resnet18-5c106cde.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            # load pretrained model:
            if pretrained:
                resnet.load_state_dict(torch.load("/media/ignored/resnet34-333f7ec4.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
def ResNet18_OS8(pretrained = False):
    return ResNet_BasicBlock_OS8(num_layers=18,pretrained=pretrained)

def ResNet34_OS8(pretrained = False):
