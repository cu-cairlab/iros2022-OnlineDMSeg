...
def inference(imgProcessor,image_result,imname):
    ...
    infectedArea,leaveArea,assembledImg = imgProcessor.inference(image_result,dump=isOutput)
    print('check2')
...
def getImg(cam_list,cam):
    #global image_result
    grab_next_image_by_trigger(cam_list,None)
    
    image_result2 = cam.GetNextImage(5000)
    if image_result2.IsIncomplete():
        ...
        width = image_result2.GetWidth()
        height = image_result2.GetHeight()
        print('Camera grabbed image, width = %d, height = %d' % (width, height))
        print('check1')   
        print('check1')
        # Convert image to mono 8
        image_result2 = image_result2.Convert(PySpin.PixelFormat_RGB8)#, PySpin.HQ_LINEAR)
        print('check1')
        image_result2 = image_result2.GetData().reshape(height, width, 3)
...
def acquire_images(cam_list,dataPath,ser,imgProcessor):
    """
            ...
            try:
                ...
                for i, cam in enumerate(cam_list):
                    try:
                        ...
                        p = Process(target=getImg, args=(cam_list,cam,))
                        p.start()
                        p.join()
                        ...
                        if image_result is None or image_result.size == 0:
                            print('Image is None')
                        else:
                            ...
                            inference(imgProcessor,image_result,'%s-%d' % (device_serial_number, received_time))
...
def run_multiple_cameras(cam_list,dataPath,ser,imgProcessor):
    """
    ...
    try:
        ...
        result &= acquire_images(cam_list,dataPath,ser,imgProcessor)
...
def main():
    ...
    result = run_multiple_cameras(cam_list,dataPath,ser,imgProcessor)
