...
def main():
  ...
  if args.dataset == 'pascal':
    ...
    dataset = DMData(args.dataPath,train=False,test=False, inference=True)
  ...
  with torch.no_grad():
    validation(model,dataset,epoch=1,folder=args.output,visualize_folder=args.visualize_dir)


def validation(model,dataset,epoch,folder=None, visualize_folder = None):
    model.eval()
    if folder!= None:
    ...
    needVisualize = False
    if visualize_folder!= None and visualize_folder != '':
      needVisualize = True
    ...
    for i in range(len(dataset)):      
      ...
      imname = dataset.images[i].split('/')[-1][:-4]+'_prediction.png'
      ...
      #mask_pred.save(os.path.join(folder, imname))
          ...
          cv2.imwrite(os.path.join(visualize_folder, imname),img)

      cv2.imwrite(folder+'/'+imname,pred,[cv2.IMWRITE_PNG_COMPRESSION, 0])
