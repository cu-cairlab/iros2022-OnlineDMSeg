...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        #============================================
        self.extra1 = self._make_extra_layer(block, 1,fix_kernel_size)
        self.extra2 = self._make_extra_layer(block, 2,fix_kernel_size)
        self.extra3 = self._make_extra_layer(block, 3,fix_kernel_size)
        self.extra4 = self._make_extra_layer(block, 4,fix_kernel_size)
    ...
    def _make_extra_layer(self, block: Type[Union[BasicBlock, Bottleneck]], current_layer,fix_kernel_size):
        ...
        if self.feature_map_size_layer < current_layer:
                ...
                if not fix_kernel_size:
                    kernel_size = stride+1
                else:
                    kernel_size = 1
                return nn.Sequential(nn.ConvTranspose2d(input_size[current_layer-1], self.feature_channel, kernel_size=kernel_size, stride=stride),nn.ReLU(inplace=True))
