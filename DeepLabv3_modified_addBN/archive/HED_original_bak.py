...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        if self.feature_channel == 1:
            ...
            self.extra5 = nn.Sequential(nn.Conv2d((64+128+256+512), self.feature_channel, kernel_size=1, stride=1),nn.ReLU(inplace=True))
    ...
    def _make_extra_layer(self, block: Type[Union[BasicBlock, Bottleneck]], current_layer):
        ...
        if self.feature_channel == 1:
            ...
            if self.feature_map_size_layer == current_layer:
                return nn.Sequential(nn.Conv2d(input_size[current_layer-1], input_size[current_layer-1], kernel_size=1, stride=1),nn.ReLU(inplace=True))
            if self.feature_map_size_layer > current_layer:
                if (self.feature_map_size_layer-current_layer) == 1:
                    return nn.Sequential(nn.Conv2d(input_size[current_layer-1], input_size[current_layer-1], kernel_size=3, stride=2, padding = 1),nn.ReLU(inplace=True))
                if (self.feature_map_size_layer-current_layer) == 2:
                    return nn.Sequential(nn.Conv2d(input_size[current_layer-1], input_size[current_layer-1], kernel_size=3, stride=2, padding = 1),nn.ReLU(inplace=True),
                                         nn.Conv2d(input_size[current_layer-1], input_size[current_layer-1], kernel_size=3, stride=2, padding = 1),nn.ReLU(inplace=True))
                if (self.feature_map_size_layer-current_layer) == 3:
                    return nn.Sequential(nn.Conv2d(input_size[current_layer-1], input_size[current_layer-1], kernel_size=3, stride=2, padding = 1),nn.ReLU(inplace=True),
                                         nn.Conv2d(input_size[current_layer-1], input_size[current_layer-1], kernel_size=3, stride=2, padding = 1),nn.ReLU(inplace=True),
                                         nn.Conv2d(input_size[current_layer-1], input_size[current_layer-1], kernel_size=3, stride=2, padding = 1),nn.ReLU(inplace=True))
            if self.feature_map_size_layer < current_layer:
                ...
                stride = int(2**(current_layer-self.feature_map_size_layer))
                kernal_size = 2*stride
                return nn.Sequential(nn.Conv2d(input_size[current_layer-1], input_size[current_layer-1], kernel_size=1, stride=1),nn.ConvTranspose2d(input_size[current_layer-1], input_size[current_layer-1], kernel_size=kernal_size, stride=stride),nn.ReLU(inplace=True))
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        if self.feature_accumumation_start == 2:
            ...
            x22 = self.extra2(x)
            #x2 = torch.add(x2,nn.functional.pad(x22,(0,x2.size()[3]-x22.size()[3],0,x2.size()[2]-x22.size()[2])))
            x2 = torch.cat((x2,nn.functional.pad(x22,(0,x2.size()[3]-x22.size()[3],0,x2.size()[2]-x22.size()[2]))),1)
            '''
            print("layer 2")
            print("x2 size:")
            print(x2.size())
            ...
            print(x22.size())
            print("padding:")
            print((0,x2.size()[3]-x22.size()[3],0,x2.size()[2]-x22.size()[2]))
            '''

            if self.multiLoss:
                out2 = nn.functional.pad(x22,(0,x2.size()[3]-x22.size()[3],0,x2.size()[2]-x22.size()[2]))
            #print(x2.size())
        x = self.layer3(x)
        ...
        if self.feature_accumumation_start == 3:
            ...
            x2 = torch.cat((x2,nn.functional.pad(x22,(0,x2.size()[3]-x22.size()[3],0,x2.size()[2]-x22.size()[2]))),1)
            if self.multiLoss:
                out3 = nn.functional.pad(x22,(0,x2.size()[3]-x22.size()[3],0,x2.size()[2]-x22.size()[2]))
            #print(x2.size())
        ...
        if self.feature_accumumation_start == 4:
            ...
            #print(x2.size())
            if self.multiLoss:
                out4 = nn.functional.pad(x22,(0,x2.size()[3]-x22.size()[3],0,x2.size()[2]-x22.size()[2]))
            '''
            print("layer 4")
            print("x2 size:")
            print(x2.size())
